...
void main() {
  group('ringPointForProgress', () {
    ...
    test('100% returns to the top, same as 0%', () {
      final start = ringPointForProgress(
        progress: 0,
        center: center,
        radius: radius,
      );
      final end = ringPointForProgress(
        progress: 1.0,
        center: center,
        radius: radius,
      );
    ...
    test(
      'clamps out-of-range progress instead of extrapolating past a full turn',
      () {
        final over = ringPointForProgress(
          progress: 1.4,
          center: center,
          radius: radius,
        );
        final atOne = ringPointForProgress(
          progress: 1.0,
          center: center,
          radius: radius,
        );
        ...
        final under = ringPointForProgress(
          progress: -0.5,
          center: center,
          radius: radius,
        );
        final atZero = ringPointForProgress(
          progress: 0.0,
          center: center,
          radius: radius,
        );
    ...
    test(
      'every point stays exactly radius away from center regardless of progress',
      () {
        for (var i = 0; i <= 20; i++) {
          final progress = i / 20;
          final p = ringPointForProgress(
    ...
    test(
      'matches the prototype formula literally: angle = -90 + progressPercent/100*360',
      () {
  ...
  testWidgets('RaceRing renders with racer markers without throwing', (
    tester,
  ) async {
