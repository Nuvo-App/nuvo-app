...
| [`15-crew-system-plan.md`](15-crew-system-plan.md) | **PLAN ONLY** — the crew connection graph (build on `crew_connections` / `member_passes`), request/accept lifecycle, privacy, crew leaderboard, race invites; Squads (named groups) sketched as a follow-on. |
| [`16-notification-system-plan.md`](16-notification-system-plan.md) | **PLAN ONLY** — notification categories & triggers, `notifications` table, `emitNotification` inline-from-route, FCM push + device tokens, preferences, rate limits, in-app centre. |
| [`17-qr-sharing-plan.md`](17-qr-sharing-plan.md) | **PLAN ONLY** — one signed-token universal-link invite model for race + crew (+ squad later), `get.nuvo.app` / AASA, preview-before-join, QR scanner, personal QR, deep-link cold start. |
