...
| F | `device_tokens` + `POST/DELETE /devices`; `domain/push.ts` FCM HTTP v1 sender (dormant until `FCM_SERVICE_ACCOUNT` + `FCM_PROJECT_ID` secrets); `safeEmit(c)` = emit + best-effort push off the response path. Flutter `PushService` (guards `Firebase.initializeApp()`, no-ops if unconfigured) — contextual permission (after first race join / crew connect), token sync on sign-in, unregister on sign-out, foreground → inbox refresh, tap → `NuvoDestination` → router. iOS `aps-environment` entitlement added. | **scaffold done, DORMANT** — needs `GoogleService-Info.plist` + `google-services.json` + APNs `.p8` in Firebase + the two worker secrets |
| G | AASA + `assetlinks.json` + `/j/:token` web fallback all served by the Worker. | **served** — remaining: Apple portal "Associated Domains" + "Push Notifications" toggles for `net.getnuvo.app`; Android release keystore SHA-256 into `assetlinks.json`; App Store id |
| H | `/settings/notifications` — grouped in-app + push toggles per category (worker `GET/PATCH /notifications/preferences`), reached from the inbox. `POST /invites` rate-limited 20/user/hr; invite-token shape guard; blocking on every accept path. | **done** (prefs screen + the rate limits already in place; a dedicated diagnostics-events layer is a follow-up) |
